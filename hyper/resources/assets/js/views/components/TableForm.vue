  ...
  <div class="animated fadeIn">
    <div class="row">
      ...
      <div class="col-sm-6">
        <b-card>
          <div slot="header">
            <strong>{{ ventana }}</strong> <small>Mesa</small>
          </div>
          <form action="" >
...
export default {
  ...
  mounted: function() {
    ...
    if (this.ruta === 'EditarMesa') {

      this.denyAccessIfNotAuthorized ('editar', 'Dashboard', 'No tienes permisos para ver a las categorias.');

      this.ventana = 'Editar';

      this.$store.dispatch('tableById', { id: this.$route.params.id})
        .then(({data}) => {

           this.name = data.tableById.name; 

        });
    }

    if(this.ruta === 'CrearMesa'){
  ...
  methods: {
    displayError(res) {
      ...
      VueOnToast.ToastService.pop('error', 'Error al crear mesa', 'Corrija para poder crear con éxito');
    ...
    submit() {
        ...
        .then(res => {
            if(res.errors) {
              ...
              VueOnToast.ToastService.pop('success', 'Mesa creada', 'La mesa fue creado con éxito.');
            }
        });
    ...
    edit() {
        ...
        .then(res => {
            if(res.errors) {
              ...
              VueOnToast.ToastService.pop('success', 'Mesa editada', 'La mesa fue editada con éxito.');
            }
        });
