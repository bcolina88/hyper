  ...
  <div class="wrapper">
    <div class="animated fadeIn">
      <div class="row">
        ...
        <router-link :to="{ name: 'CrearTasa'}">
          <b-button variant="success"  v-if="permissionGenerator('agregar',maestroo)"><i class="fa fa-plus"></i>&nbsp; Nueva Tasa</b-button>
        </router-link>
        ...
        <b-card header="Listado de tasas">
          ...
          <div>
            ...
            <b-table responsive :items="tasas" :fields="fields" :current-page="currentPage" :per-page="perPage">
              ...
              <template slot="options" slot-scope="row">
                ...
                <router-link :to="{ name: 'EditarTasa', params: { id: row.item.id }}">
...
function Tasas({  id, value, active, currency}) {

  this.id = id;
  this.value = value;
  this.active = active;
  this.currency = currency.description;
}
...
export default {
  ...
  computed: {
    hasRecords() {
      return this.tasas.length > 0;
    },
    totalRows() {
      return this.tasas.length;
    }
  },
  methods: {
    ...
    read() {
      ...
      this.$store.dispatch('getTasas', null).then(({data}) => {

        data.rates.forEach(tasa => {
          this.tasas.push(new Tasas(tasa));
        });
      });
    },
    del(id) {
      this.$store.dispatch('deleteRate', { id: id}).then(({data}) => {
        VueOnToast.ToastService.pop('success', 'Tasa Eliminada', 'La tasa fue eliminada con éxito.');
        this.tasas = [];
        this.read();
      });
    },
    ...
    activeTable(id,value) {
      ...
      this.$store.dispatch('blockedRate', { id: id, status : status}).then(({data}) => {
        VueOnToast.ToastService.pop('success', 'Estatus cambiado', 'La tasa cambio de estatus con éxito.');
        this.tasas = [];
        this.read();
      });
    },
