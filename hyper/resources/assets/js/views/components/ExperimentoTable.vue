...
Experimento</b-button>
        ...
        <b-card header="Listado de experimentos">
            ...
            <b-table responsive :items="ventas" :fields="fields" :current-page="currentPage" :per-page="perPage">
             ...
             <template slot="active" slot-scope="row">
              ...
              <template slot="options" slot-scope="row">
                ...
                <router-link :to="{ name: 'EditarExperimento', params: { id: row.item.id }}">
    ...
    <b-modal id="myModal" title="Eliminar"  @ok="del(selectedUser.id)" class="text-center">
      <h6>Eliminar experimento #{{selectedUser.id}}</h6>
      <h6>¿Estas seguro que deseas eliminar este experimento #{{selectedUser.id}} ?</h6>
    </b-modal>
...
export default {
  ...
  mounted: function() {
    ...
    this.denyAccessIfNotAuthorized ('ver', 'Dashboard', 'No tienes permisos para ver a los experimentos.');
  },
  computed: {
    hasRecords() {
  ...
  methods: {
    ...
    read() {
      ...
      axios.get("api/Experimentos",null).then(res => {
    ...
    del(id) {
      this.$store.dispatch('deleteExperimento', { id: id}).then( data => {
        VueOnToast.ToastService.pop('success', 'El Experimento  fue eliminado', 'El Experimento fue eliminado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    ...
    status(id,value) {
      ...
      this.$store.dispatch('blockedExperimento', { id: id, status : status}).then( data => {
        VueOnToast.ToastService.pop('success', 'Estatus cambiado', 'El Experimento cambio de estatus con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    completar(id,value1) {
      ...
      this.$store.dispatch('completExperimento', { id: id, status : status}).then( data => {
        VueOnToast.ToastService.pop('success', 'Estado cambiado', 'El experimento cambio de estado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
