  ...
  <div class="wrapper">
    <div class="animated fadeIn">
      <div class="row">
        ...
        <router-link :to="{ name: 'CrearPalanca'}">
...
Palanca</b-button>
        ...
        <b-card header="Listado de palancas">
            ...
            <b-table responsive :items="ventas" :fields="fields" :current-page="currentPage" :per-page="perPage">
             ...
             <template slot="active" slot-scope="row">
              ...
              <template slot="options" slot-scope="row">
                ...
                <router-link :to="{ name: 'EditarPalanca', params: { id: row.item.id }}">
    ...
    <b-modal id="myModal" title="Eliminar"  @ok="del(selectedUser.id)" class="text-center">
      <h6>Eliminar objetivo #{{selectedUser.id}}</h6>
      <h6>¿Estas seguro que deseas eliminar esta palanca #{{selectedUser.id}} ?</h6>
    </b-modal>
...
function Ventas({ id,name, objetivo, active, completada, num_exp_valid}) {

  this.id = id;
  this.name = name;
  this.objetivo = objetivo.name;
  this.active = active;
  this.completada = completada;
  this.num_exp_valid =  num_exp_valid;
...
export default {
  ...
  methods: {
    ...
    read() {
      ...
      axios.get("api/Palancas",null).then(res => {
    ...
    del(id) {
      this.$store.dispatch('deletePalanca', { id: id}).then( data => {
        VueOnToast.ToastService.pop('success', 'El objetivo  fue eliminado', 'La palanca fue eliminado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    ...
    status(id,value) {
      ...
      this.$store.dispatch('blockedPalanca', { id: id, status : status}).then( data => {
        VueOnToast.ToastService.pop('success', 'Estatus cambiado', 'La palanca cambio de estatus con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    completar(id,value) {
      ...
      this.$store.dispatch('completPalanca', { id: id, status : status }).then( data => {
        VueOnToast.ToastService.pop('success', 'Estado cambiado', 'El palanca cambio de estado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
