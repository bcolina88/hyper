  ...
  <div class="wrapper">
    <div class="animated fadeIn">
      <div class="row">
        ...
        <router-link :to="{ name: 'CrearObjetivo'}">
...
Objetivo</b-button>
        ...
        <b-card header="Listado de objetivos">
          ...
          <div>
            ...
            <b-table responsive :items="ventas" :fields="fields" :current-page="currentPage" :per-page="perPage">
              ...
              <template slot="options" slot-scope="row">
                ...
                <router-link :to="{ name: 'EditarObjetivo', params: { id: row.item.id }}">
    ...
    <b-modal id="myModal" title="Eliminar"  @ok="del(selectedUser.id)" class="text-center">
      ...
      <h6>¿Estas seguro que deseas eliminar este objetivo #{{selectedUser.id}} ?</h6>
    </b-modal>
...
function Ventas({ id,name, fecha_inicial, fecha_fin, proyecto, active, completada}) {

  this.id = id;
  this.name = name;
  this.fecha_inicial = fecha_inicial;
  this.fecha_fin = fecha_fin;
  this.proyecto = proyecto.name;
  this.active = active;
...
export default {
  ...
  methods: {
    ...
    read() {
      ...
      axios.get("api/Objetivos",null).then(res => {
    ...
    del(id) {
      this.$store.dispatch('deleteObjetivo', { id: id}).then( data => {
        VueOnToast.ToastService.pop('success', 'El objetivo  fue eliminado', 'El objetivo fue eliminado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    ...
    status(id,value) {
      ...
      this.$store.dispatch('blockedObjetivo', { id: id, status : status}).then( data => {
        VueOnToast.ToastService.pop('success', 'Estatus cambiado', 'El objetivo cambio de estatus con éxito.');
        this.ventas = [];
        this.read();
      });
    },
    completar(id,value1) {
      ...
      this.$store.dispatch('completObjetivo', { id: id, status : status}).then( data => {
        VueOnToast.ToastService.pop('success', 'Estado cambiado', 'El objetivo cambio de estado con éxito.');
        this.ventas = [];
        this.read();
      });
    },
