  ...
  <div class="wrapper">
    <div class="animated fadeIn">
      <div class="row">
        ...
        <router-link :to="{ name: 'CrearCategoria'}">
          <b-button variant="success" v-if="permissionGenerator('agregar',maestroo)"><i class="fa fa-plus"></i>&nbsp; Nueva categoria</b-button>
        </router-link>
        ...
        <b-card header="Listado de categorias">
          ...
          <div>
            ...
            <b-table responsive :items="categorys" :fields="fields" :current-page="currentPage" :per-page="perPage">
              ...
              <template slot="options" slot-scope="row">
                ...
                <router-link :to="{ name: 'EditarCategoria', params: { id: row.item.id }}">
    ...
    <b-modal id="myModal" title="Eliminar"  @ok="del(selectedCategory.id)" class="text-center">
      <h6>Eliminar categoria {{selectedCategory.name}}</h6>
      <h6>¿Estas seguro que deseas eliminar esta categoria {{selectedCategory.name}} ?</h6>
    </b-modal>
...
function Categorias({  id, name, active}) {
...
export default {
  ...
  methods: {
    ...
    read() {
      this.$store.dispatch('getCategorias', null).then(({data}) => {
        data.categorys.forEach(category => {
          this.categorys.push(new Categorias(category));
        });
      });
    },
    del(id) {

      this.$store.dispatch('deleteCategory', { id: id}).then(({data}) => {
        VueOnToast.ToastService.pop('success', 'Categoria Eliminada', 'La categoria fue eliminada con éxito.');
        this.categorys = [];
        this.read();
      });
    },
    ...
    status(id,value) {
      ...
      this.$store.dispatch('blockedCategory', { id: id, status : status}).then(({data}) => {
        VueOnToast.ToastService.pop('success', 'Estatus cambiado', 'La categoria cambio de estatus con éxito.');
        this.categorys = [];
        this.read();
      });
    },
